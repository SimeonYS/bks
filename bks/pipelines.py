...
class BksPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bks`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bks WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bks (date, title, link, content)"
